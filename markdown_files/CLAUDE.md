...
> ## ⇨ START HERE
>
> This file is the map and the standing rules. It lives in **`markdown_files/`** together with the
> rest of the tracked documentation; the root `CLAUDE.md` is a pointer that keeps auto-loading
> working. The detail lives in the sibling documents — start from
> [`INDEX.md`](INDEX.md), which says which file answers which question.
>
> | document | what it answers |
> |---|---|
> | [`INDEX.md`](INDEX.md) | **which file to open** — the taxonomy, and the recurring topics with their one authoritative home |
> | [`MODEL.md`](MODEL.md) | the maths: σ-tensors, the global residual term by term, nonlinear pressure, Jacobians |
> | [`ARCHITECTURE.md`](ARCHITECTURE.md) | code structure and workflow: stacked layout, `src/` map, FE spaces, time loops, distributed path |
> | [`VERIFIED_SCOPE.md`](VERIFIED_SCOPE.md) | **is it correct, and how far does the claim reach?** — the MMS campaign, the Jacobian oracle, scope boundaries |
> | [`TEST_SUITE.md`](TEST_SUITE.md) | **what is checked, and what would slip through?** — every gate and its blind spots |
> | [`OPEN_ISSUES.md`](OPEN_ISSUES.md) | **what is known wrong or unexplained?** — defects in work already done |
> | [`PLANNED_CAMPAIGNS.md`](PLANNED_CAMPAIGNS.md) | **what runs next?** — the MMS campaigns, their ladders, and the code they need |
> | [`COMPLETED_VBASIS_STUDY.md`](COMPLETED_VBASIS_STUDY.md) | the design record of the finished vertical-basis study |
...
> | [`OUTPUT_NAMING_PROPOSAL.md`](OUTPUT_NAMING_PROPOSAL.md) | the output-directory grammar — implemented in `output_dir_name` |
>
