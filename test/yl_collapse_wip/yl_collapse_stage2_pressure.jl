# ==============================================================
#  yl_collapse_stage2_pressure.jl
#
#  STAGE 2, REDONE AS AN OPERATOR-VS-OPERATOR COMPARISON (the stage-1 method).
#
#  The previous stage 2 compared OUR package against Dw/Dt -- a PHYSICS check (does the
#  vertical momentum equation hold?). Useful, but it is not what this chapter is for: the
#  chapter verifies that our p=1 reduction reproduces the LFE-M results PUBLISHED by
#  Yang & Liu. So stage 2 should do exactly what stage 1 does, on the pressure:
#  evaluate BOTH closed-form profiles pointwise in sigma, over several vertical meshes and
#  several horizontal stations, and compare.
#
#  OURS   (eq: pnh FE approx):  p_nh = rho*H * [ sum_j L_j . Pi_j(s) + sum_kj N_kj . Lam_kj(s) ]
#         with Pi_j = int_s^1 theta_j, Lam_kj = int_s^1 Theta_kj. Evaluating those integrals
#         IS evaluating our closed form -- the integrands are piecewise polynomial in sigma,
#         so a composite Gauss rule per element is exact, not an approximation.
#
#  THEIRS (<2.22>+<2.23>, supplementary B): p~ = rho*H * sum_{m=0..4} P_m sigma^m, with the
#         two conventions the chapter records: (a) the published p_{k,n} are coefficients of
#         -p_nh/(rho H), and (b) the surface condition <2.5> already supplies the hydrostatic
#         share of P_0, so writing P_0 = g + C double-counts gravity.
#
#  THEIRS IS THE FULL PRESSURE, OURS IS THE NON-HYDROSTATIC PART. In sigma coordinates
#  z = -h + sigma*H, so the hydrostatic head is rho*g*(eta - z) = rho*g*H*(1-sigma).
#  Both are reported: p_nh (subtracting it from theirs) and p_full (adding it to ours).
# ==============================================================
# ===========================================================================
...
# ---------------- STAGE 2 AS AN OPERATOR COMPARISON -------------------------
const RHO = 1.0        # cancels: both sides carry the same factor

"OUR p_nh(sigma) = rho*H * int_s^1 [ sum L.theta + sum N.Theta ] ds'  (exact per element)."
function pnh_ours(c, x, t, s)
    H = Hc(x, t)
    acc = 0.0
    M = length(c) - 1
    for k in 1:M                                    # integrate element by element from s to 1
        a = max(s, c[k]); b = c[k+1]
        b > a && (acc += gquad(sp -> package_fixed(c, x, t, sp), c, a, b))
    end
    return RHO * H * acc
end

"THEIR full pressure p~(sigma) = rho*H*sum_m P_m sigma^m, from <2.22>+<2.23>."
function pfull_theirs(c, x, t, s)
    H  = Hc(x, t)
    Pc = Pfull(c, x, t)
    k  = clamp(searchsortedlast(c, s), 1, length(c) - 1)
    return RHO * H * sum(Pc[k, n+1] * s^n for n in 0:4)
end

hydrostatic(c, x, t, s) = RHO * GRAV * Hc(x, t) * (1 - s)
pnh_theirs(c, x, t, s)  = pfull_theirs(c, x, t, s) - hydrostatic(c, x, t, s)

function stage2_pressure()
    println("STAGE 2 (operator comparison) - our p_nh closed form vs Yang & Liu <2.22>+<2.23>\n")
    @printf("%-22s %-6s %14s %12s %12s\n", "mesh", "x", "worst |dp_nh|", "|p_nh| scale", "relative")
    println("-"^70)
    meshes = (("M=2  c2=0.728", [0.0,0.728,1.0]), ("M=2  c2=0.5", [0.0,0.5,1.0]),
              ("M=3  (0.4,0.8)", [0.0,0.4,0.8,1.0]), ("M=4  (0.3,0.6,0.85)", [0.0,0.3,0.6,0.85,1.0]))
    stations = (0.0, 3.7, 11.3, 12.0, 19.4)
    worst = 0.0; worst_full = 0.0
    for (lab, cc) in meshes
        CMESH[] = cc
        for x in stations
            dmax = 0.0; pmax = 0.0; dfull = 0.0; pfmax = 0.0
            for s in range(1e-9, 1-1e-9; length = 501)
                a = pnh_ours(cc, x, T0, s); b = pnh_theirs(cc, x, T0, s)
                dmax = max(dmax, abs(a-b)); pmax = max(pmax, abs(b))
                af = a + hydrostatic(cc, x, T0, s); bf = pfull_theirs(cc, x, T0, s)
                dfull = max(dfull, abs(af-bf)); pfmax = max(pfmax, abs(bf))
            end
            rel = dmax/max(pmax, 1e-300)
            worst = max(worst, rel); worst_full = max(worst_full, dfull/max(pfmax,1e-300))
            @printf("%-22s %-6.1f %14.3e %12.3e %12.3e\n", lab, x, dmax, pmax, rel)
        end
    end
    @printf("\nSTAGE 2 WORST RELATIVE DIFFERENCE (p_nh)  : %.3e\n", worst)
    @printf("STAGE 2 WORST RELATIVE DIFFERENCE (p_full): %.3e\n", worst_full)
end
