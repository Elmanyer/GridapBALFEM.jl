#!/bin/bash
# ==============================================================
# 1-D NONLINEAR DIAGNOSTIC — nl_full_flat with an EXACT (AD) JACOBIAN
#
...
# ==============================================================
#  DIAGNOSTIC OVERRIDES — appended after the production settings above, so this
#  run differs from run_1dprod_nl_full_flat.sh in EXACTLY TWO knobs.
#
#  WHY. The production run stalled at t = 12.6 s: Newton hit its 50-iteration cap
#  at a residual of 1.92e-05 against nl_tol = 1e-05, with the iteration count
#  climbing 6 -> 6 -> 12 -> 12 -> 19 -> cap over the preceding 2 s, while eta
#  stayed BOUNDED at ~0.12 m. The :native control in the same batch held 4-5
#  iterations/step at the same time and amplitude.
#
#  That is the signature of the KNOWN quasi-Newton gap, not of an unstable
#  operator: jacobian_u omits the {1,2,4,5} pressure blocks that nl_pressure=:full
#  adds to the RESIDUAL (OPEN_ISSUES.md §2, CLAUDE.md §5.7 item 5), and the gap has
#  a measured cliff in amplitude -- Newton stalls at 9.2e-04 at a_eta=0.8 and only
#  falls to ~2e-09 at a_eta <= 0.4. This run is at A = 0.10 m.
#
#  BALFEM_USE_AD=1 differentiates the SAME residual exactly, so it separates the
#  two readings (rule 17b: a converged Newton step is a property of the residual,
#  not the Jacobian -- an incomplete Jacobian changes the PATH and the COST, never
#  the root):
#     * completes past t = 12.6 s  => SOLVE defect: the operator is fine, the hand
#       Jacobian cannot track it at this amplitude;
#     * stalls or diverges at the same time => OPERATOR defect, and the :full
#       tier is genuinely unstable here.
#
#  DURATION cut to 20 periods (32 s). AD is substantially slower per assembly and
#  the question is settled by t ~ 16 s; the full 100-period integration is the
#  production run's job, not the diagnostic's.
export BALFEM_USE_AD=1
export BALFEM_PERIODS=20
