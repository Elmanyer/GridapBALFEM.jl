...
# ==============================================================
#  BAR SHOULDER LADDER — this run: shoulder = 1.5 m, max|grad h| = 0.67 (34 deg)
#
#  THE BRACKET SO FAR, all at the SAME height (2.0 m), span (26-34 m) and crest
#  depth (1.5 m) -- only the shoulder length differs:
#     shoulder 0.5 m  (2:1, 63 deg)  -> DIED t = 37.2 s, growing mode pinned at
#                                       the downwave shoulder, Newton 8->25->cap
#     shoulder 2.0 m  (1:2, 27 deg)  -> STABLE past t = 80 s, eta flat at 0.1043
#  So the threshold lies between them. But 2.0 m also lost the EFFECT: over 205
#  post-transit diagnostic samples the global max never once lands on the bar,
#  against a clear 0.125 on the crest for the square one. Stability and
#  visibility are in tension and the useful mesh is in between.
#
#  max|grad h| = hbar / (2*sramp), so this run sits at 0.67 (34 deg).
#
#  ⚠ READ THIS LADDER AGAINST THE FLAT-BED CONTROL, not against a remembered
#  baseline: P1LFE-2_1d_bcplane_nl_native_flat_Q2Q1-nx240_A0.1_T1.6 completed
#  100 periods with eta settling 0.10494 -> 0.10300 and Newton flat at 5.12.
# ==============================================================
export BALFEM_SBAR=1.5
export BALFEM_OUTDIR=output/local_1d/P1LFE-2_1d_bcplane_nl_native_bar_Q2Q1-nx240_A0.1_T1.6_shoulder1.5
