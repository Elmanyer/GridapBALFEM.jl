#!/bin/bash
#  ==== 2026-09-23 TEST A (Q2/Q1 twin): identical to run_1dc3v_base_mixed.sh (aux in Q2, >=63.4 periods flat)
#  except the AUXILIARY SPACE ORDER: BALFEM_P_AUX=1 (the eta order). Pairs with run_1dc3q_base_mixed_aux2.sh ====
# ==============================================================
...
export BALFEM_OUTDIR=output/local_1d/c3v_base_mixed_aux1
...
export BALFEM_P_AUX=1          # <- THE ONE VARIED KNOB (aux was Q2 = velocity order)

# ---- CONFIGURATION 'base' (mixed arm) ------------------------------------------
#  BASELINE: dx=0.25, dt=0.04, A=0.10, T=1.6 -- the configuration whose PROJECTED arm
#  dies at t = 12.00 s. The projected arm is NOT repeated here: it is already recorded
#  (output/local_1d/c3iso_gs) and a 100-period run of something that dies at 12 s is waste.
#  
