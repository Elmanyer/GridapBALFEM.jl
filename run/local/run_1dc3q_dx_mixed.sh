...
export BALFEM_OUTDIR=output/local_1d/c3q_dx_mixed
...
# ---- CONFIGURATION 'dx' (mixed arm) ------------------------------------------
#  RESOLUTION: nx = 480 (dx = 0.125). Rule 38b's axis -- refinement ADVANCES the projected
#  onset (measured 12.60 -> 5.00 s at dt=0.04). The strongest single discriminator:
#  a recovery error gets worse with h, a physical mode does not.
#  Ly = dx keeps the single cross-cell isotropic (rule 12/22).
#  
...
export BALFEM_LY=0.125

# ---- Q3/Q2 batch, configuration 'dx' ----------------------------------------
