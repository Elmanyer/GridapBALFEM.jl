#!/bin/bash
#  TH5 - Q2/Q1, EXPLICIT RK4 (Yang & Liu's integrator). RK4 damps as (k*dt)^6 at the grid scale -- the sink an L-stable implicit scheme does not provide. dt cut 4x for the explicit CFL.
# ==============================================================
...
export BALFEM_TABLEAU=EXRK_RungeKutta_4_4
#  dt = 0.005: EXPLICIT CFL. The smoke test ran at dt=0.01 on dx=0.50; the campaign
#  mesh is dx=0.25, and an explicit scheme's limit tightens with dx, so this halves it.
...
export BALFEM_OUTDIR=output/local_1d/nonlinear_stability_taylor_hood/integrators/th_q2q1_rk4
