#!/bin/bash
# ==============================================================
#  CLASS-III TREATMENT FACTORIAL — MIXED (PROJECTION-FREE) grad-S -- the decisive comparison, on the REAL flume
#
...
export BALFEM_OUTDIR=output/local_1d/c3q3q2_mixed
...
# ---- PROJECTION-FREE ---------------------------------------------------------
#  ⚠ THIS MUST PRECEDE `balfem_local_run` (rule 38h).
#  grad-S is promoted to a genuine unknown (5 fields) instead of a frozen L2 projection.
#  Everything else is IDENTICAL to run_1dc3iso_gs.sh, which is therefore the like-for-like
#  PROJECTED control: it began diverging at t = 12.00 s with u_max 0.61 -> 3.17 and
#  x_at_max pinned at 0.25 m (the inflow).
#
#  ⚠ COST: AD Jacobians on 5 fields. The 7-field version measured ~180 s/step at nx=24 in a
#  6 m box; this is nx=240. Expect this to be SLOW and to need babysitting.
#  ⚠ READ THE LOCATION, NOT JUST THE TIME. `x_at_max` only became available on the mixed
#  layout today (test_diagnostics_mixed.jl). The projected failure pins at the INFLOW; a
#  mixed failure somewhere else is a different mode, not the same one delayed.
export BALFEM_MIXED=1

# ---- Q3/Q2 ------------------------------------------------------------------
#  ⚠ THIS PAIR NEEDS ITS OWN CONTROL. The projected gs control that died at t = 12.00 s is
#  Q2/Q1; there is NO Q3/Q2 projected run anywhere (it was factorial cell #1 and was never
#  started). So run_1dc3q3q2_proj.sh and run_1dc3q3q2_mixed.sh are launched TOGETHER and
#  read only against each other (rule 38c: the null-treatment control goes in the SAME
#  batch, never against a remembered baseline).
#  ⚠ The Q3/Q2 onset is UNKNOWN and need not be 12.6 s: onset moves with dx and the pairing,
#  and every recorded :full onset is Q2/Q1. "Later than 12.6" is therefore NOT evidence of
#  anything by itself -- only the proj-vs-mixed DIFFERENCE at matched settings is.
