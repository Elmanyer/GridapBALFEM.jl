...
export BALFEM_OUTDIR=output/local_1d/c3q_amp_mixed
...
# ---- CONFIGURATION 'amp' (mixed arm) ------------------------------------------
#  AMPLITUDE: A = 0.15 (1.5x the baseline). Steeper waves -> the O(A^2)/O(A^3) Class-III
#  terms are ~2.3x/3.4x larger, so if the projection is the culprit this should fail
#  EARLIER than the baseline in the projected arm.
#  
