#!/bin/bash
# SMALL run — BC-generated NONLINEAR :full plane wave, SHALLOW (d=0.85), A=0.1, 30 periods.
#
#  THE DIRECT CONTROL for the deep :full run that grows. Same tier, same
#  amplitude, same generation, same sponge -- only kd changes, from 3.527 to
#  1.079, crossing the Benjamin-Feir threshold. If the growth is modulational
#  instability it vanishes here; if it survives, the cause is not BF and the
#  numerics (or the quasi-Newton Jacobian) are implicated instead.
...
#SBATCH --job-name="BALFEM_nl_full_bcplane_shallow"
...
export BALFEM_OUTDIR=$HOME/GridapBALFEM.jl/output/small_bcplane_nonlinear_full_shallow_d0.85_A0.1_T2.0_P1LFE-2
